...
export default function AdminDevelopers() {
  ...
  useEffect(() => {
    ADMIN_SERVICE.listUsers()
      .then((res) => {
        ...
        setRows(users.filter((user) => user.role === 'developer'));
      })
  ...
  return (
    <div className="overflow-hidden rounded-xl bg-white shadow-sm">
      <h2 className="border-b px-4 py-3 text-lg font-semibold">Developers</h2>
      ...
      <table className="min-w-full text-left text-sm">
        ...
        <tbody>
          ...
          {!rows.length && !error && (
            <tr>
              <td colSpan={4} className="px-4 py-8 text-center text-gray-500">No developer accounts yet.</td>
            </tr>
          )}
        </tbody>
      </table>
